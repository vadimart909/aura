...
import { AURA_SERVICES } from './services'

/**
 * AuraHeader — общая шапка всех приложений Ауры.
 *
 * Одна разметка на оба сервиса: при переходе Сценарии ↔ Сегменты шапка не
 * должна дёргаться, поэтому она живёт в shared, а не копируется по приложениям.
 *
 * @param {object} props
 * @param {string} props.service — ключ текущего сервиса из AURA_SERVICES
 *
 * Активен всегда пункт текущего сервиса — не путь. Раньше каждая копия шапки
 * сравнивала pathname точно, и на внутренних страницах
 * (/scenario/view/16, /segment/create) не подсвечивался ни один пункт.
 *
 * Обёртки `__desktop` намеренно нет: ДС прячет её медиазапросом ýже 1024px,
 * подменяя блоком `__adaptive`, в котором навигации нет вообще. Без обёртки
 * шапка видна на любой ширине, а `__content` сохраняет свои 74px высоты — на
 * них завязан сдвиг алерта в CreateScenarioCanvas.css. В CSS ДС `__desktop`
 * используется только как переключатель видимости, ни один селектор на неё
 * не опирается, так что убрать её безопасно.
 */
export default function AuraHeader({ service }) {
    ...
    <header className="main-page-navigation-bar">
      <div className="main-page-navigation-bar__content">
        ...
        <nav className="main-page-navigation-bar__nav">
          {AURA_SERVICES.map((item) => (
            <SCINavigationButton
              key={item.key}
              label={item.label}
              isActive={item.key === service}
              onClick={
                item.key === service
                  // Свой сервис — навигация роутером, без перезагрузки.
                  ? () => navigate('/')
                  // Чужой — другое приложение, только полная перезагрузка.
